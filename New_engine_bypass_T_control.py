...
from pyCycle_elements.improved_pycycle_hex import HEX, HEX_cold
...
class SingleSpoolTurboshaft(pyc.Cycle):
    ...
    def setup(self):
        ...
        if design:
            ...
            balance.add_balance('mdot_anode_in_excess', val=50, lower=1.0, upper=20000, units='g/s', eq_units='hp',
                                rhs_name='pwr_target')
            self.connect('balance.mdot_anode_in_excess', 'sofc.mdot_anode_in_excess')
        ...
        self.set_order(
            ['fc', 'inlet', 'duct1', 'comp', 'Hex_cold', 'sofc', 'Hex', 'sofc_mixer', 'duct5', 'burner', 'turb',
             'HP_shaft', 'duct6',
        ...
        if design:
            ...
            newton.options[
                'maxiter'] = 200  # Todo: change this to 10 for quick results and converges only for DESIGN pt
            newton.options['solve_subsystems'] = True
            newton.options[
                'max_sub_solves'] = 10  # 10 #Todo: change to 5 for quick results and converges only for DESIGN pt
...
class MPSingleSpool(pyc.MPCycle):
    ...
    def setup(self):
        ...
        self.pyc_add_pnt('DESIGN', SingleSpoolTurboshaft(thermo_method='CEA', i_l=self.settings.SOFC_i_l,
                                                         ASR=self.settings.SOFC_ASR,
        ...
        for i, pt in enumerate(self.od_pts):
            self.pyc_add_pnt(pt, SingleSpoolTurboshaft(design=False, thermo_method='CEA', i_l=self.settings.SOFC_i_l,
                                                       ASR=self.settings.SOFC_ASR,
                                                       i0=self.settings.SOFC_i0))
