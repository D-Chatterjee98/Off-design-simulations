...
def run_TOC_partload_sweep_2():
    ...
    guess_map = {
        1.0: {'W': 2200, 'mdot': 20, 'i': 10000, 'HP_Nmech': 14800, 'Pt': 4.27, 'Tt': 450, 'turbPR': 3.0},
        0.9: {'W': 2200, 'mdot': 20, 'i': 8000, 'HP_Nmech': 14000, 'Pt': 4.27, 'Tt': 450, 'turbPR': 3.0},
    ...
    for k, frac in enumerate(load_fracs):
        ...
        #print(prob.get_val('TOC.sofc_mixer.Fl_O:tot:P', units='kPa')[0])
        #print(prob.get_val('TOC.sofc_mixer.Fl_O:stat:P', units='kPa')[0])
        #print(prob.get_val('TOC.sofc_mixer.Fl_O:stat:MN')[0])


'''
def run_TOC_partload_sweep_1():
    T4 = 1100
    N_cells = 40000
    power_fraction = 0.65
    PR = 20
    ...
    load_fracs = [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4]

    guess_map = {
        1.0: {'W': 3200, 'mdot': 22, 'i': 10000, 'HP_Nmech': 14800, 'Pt': 4.27, 'Tt': 256, 'turbPR': 3.0},
        #0.9: {'W': 3300, 'mdot': 22, 'i': 10000, 'HP_Nmech': 14800, 'Pt': 4.27, 'Tt': 256, 'turbPR': 3.0},
        #0.8: {'W': 2200, 'mdot': 22, 'i': 10000, 'HP_Nmech': 14800, 'Pt': 4.27, 'Tt': 256, 'turbPR': 3.0},
        0.7: {'W': 2500, 'mdot': 14, 'i': 8500, 'HP_Nmech': 13600, 'Pt': 4.27, 'Tt': 256, 'turbPR': 6},
        #0.6: {'W': 2100, 'mdot': 12,  'i': 8000, 'HP_Nmech': 13600, 'Pt': 4.27, 'Tt': 256, 'turbPR': 2.4},
        #0.5: {'W': 2000,  'mdot': 16,  'i': 8000, 'HP_Nmech': 13000, 'Pt': 4.27, 'Tt': 256, 'turbPR': 2.4},
        #0.4: {'W': 2000,  'mdot': 16,  'i': 8000, 'HP_Nmech': 13000, 'Pt': 4.27, 'Tt': 256, 'turbPR': 2.4},
    }
    ...
    for frac in load_fracs:
        ...
        prob.set_val('TOC.balance.HP_Nmech', g['HP_Nmech'], units='rpm')
        prob.set_val('TOC.fc.balance.Pt', g['Pt'], units='psi')
        prob.set_val('TOC.fc.balance.Tt', g['Tt'], units='degK')
        prob['TOC.turb.PR'] = g['turbPR']
        prob['TOC.comp.PR'] = PR
...
if __name__=="__main__":
    ...
    def plot_compressor_map_with_points(prob, design_pt='DESIGN', off_design_pts=None):
        ...
        # Plot off-design points if provided
        if off_design_pts:
            for pt in off_design_pts:
                off_design_Wc = prob[pt + '.' + comp_name + '.Wc'][0]
                off_design_PR = prob[pt + '.' + comp_name + '.PR'][0]
                plt.plot(off_design_Wc, off_design_PR, 'bo', markersize=8, label=f'Off-Design: {pt}')
