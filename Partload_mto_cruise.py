...
def problem_setup(T4, N_cells, power_fraction, PR, Des_pt, od_pts=None):
    prob = om.Problem()

    settings = EngineSettings()

    # Define the design point
    settings.SOFC_N_cells = N_cells
    design_pt = Des_pt
    settings.Power_target = power_fraction
    settings.des_alt = settings.alt_ft_dict[design_pt]
    settings.des_mach_no = settings.mach_no_dict[design_pt]
    settings.des_pwr_shaft = settings.pwr_shaft_kW[design_pt] * 1.341  # kW to hp
    settings.des_ISA_dT = settings.ISA_dict[design_pt]

    # Allow custom off-design points
    if od_pts is None:
        settings.od_pts = ['TOC']
    else:
        settings.od_pts = od_pts
    ...
    prob.set_val('DESIGN.balance.T_sofc_target', 1023, units='K')
    prob.set_val('DESIGN.balance.T4_target', T4, units='K')
    prob.set_val('DESIGN.balance.T_inlet_target', 873, units='K')
    prob.set_val('DESIGN.balance.nozz_PR_target', 1.1)
    ...
    prob['DESIGN.balance.W'] = 2000
    prob['DESIGN.balance.turb_PR'] = 2
    prob['DESIGN.balance.pt_PR'] = 4
    prob['DESIGN.balance.mdot_anode_in_excess'] = 20
    prob['DESIGN.balance.i_sofc'] = 5000
    prob['DESIGN.fc.balance.Pt'] = 5.66
    prob['DESIGN.fc.balance.Tt'] = 518.665288153
    prob['DESIGN.balance.hex_effectiveness'] = 0.8

    # Generic off-design guesses
    ...
    Pt_fc_guess = {'Cruise': 4.64, 'MTO': 14.64, 'TOC': 4.64}   # psi
    Tt_fc_guess = {'Cruise': 450, 'MTO': 527, 'TOC': 450}       # degR
    ...
    for i, pt in enumerate(mp_single_spool.od_pts):
        ...
        prob.set_val(pt + '.balance.mdot_anode_in_excess', mdot_anode_excess_guess[pt], units='g/s')
        prob.set_val(pt + '.balance.i_sofc', i_sofc_guess[pt], units='A/m**2')
        prob.set_val(pt + '.balance.HP_Nmech', HP_Nmech_guess[pt], units='rpm')
        prob.set_val(pt + '.fc.balance.Pt', Pt_fc_guess[pt], units='psi')
        prob.set_val(pt + '.fc.balance.Tt', Tt_fc_guess[pt], units='degR')
        prob[pt + '.turb.PR'] = 3.0
        prob[pt + '.comp.PR'] = PR
...
def run_partload_sweep_for_od_point(od_point):
    T4 = 1100
    N_cells = 40000
    power_fraction = 0.65
    PR = 20
    Des_pt = 'TOC'   # TOC is the design condition

    load_fracs = [1, 0.9]#, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.07]

    os.makedirs("Results", exist_ok=True)

    # Build model once: TOC is design, chosen point is off-design
    prob = problem_setup(T4, N_cells, power_fraction, PR, Des_pt, od_pts=[od_point])

    # Point-specific part-load guesses

    guess_map_by_point = {
        'Cruise': {
            1.0: {'W': 2200, 'mdot': 20.0, 'i': 8000, 'HP_Nmech': 10000, 'Pt': 4.64, 'Tt': 450, 'turbPR': 3.0},
            0.9: {'W': 2150, 'mdot': 18.0, 'i': 7600, 'HP_Nmech': 9800, 'Pt': 4.64, 'Tt': 450, 'turbPR': 3.0},
            #0.8: {'W': 2100, 'mdot': 16.0, 'i': 7200, 'HP_Nmech': 9600, 'Pt': 4.64, 'Tt': 450, 'turbPR': 3.1},
            #0.7: {'W': 2000, 'mdot': 14.0, 'i': 6800, 'HP_Nmech': 9400, 'Pt': 4.64, 'Tt': 450, 'turbPR': 3.1},
            #0.6: {'W': 1900, 'mdot': 12.0, 'i': 6200, 'HP_Nmech': 9200, 'Pt': 4.64, 'Tt': 450, 'turbPR': 3.2},
            #0.5: {'W': 1750, 'mdot': 10.0, 'i': 5500, 'HP_Nmech': 9000, 'Pt': 4.64, 'Tt': 450, 'turbPR': 3.3},
            #0.4: {'W': 1600, 'mdot': 8.5, 'i': 4700, 'HP_Nmech': 8800, 'Pt': 4.64, 'Tt': 450, 'turbPR': 3.5},
            #0.3: {'W': 1450, 'mdot': 7.0, 'i': 3800, 'HP_Nmech': 8600, 'Pt': 4.64, 'Tt': 450, 'turbPR': 3.8},
            #0.2: {'W': 1300, 'mdot': 5.8, 'i': 2900, 'HP_Nmech': 8400, 'Pt': 4.64, 'Tt': 450, 'turbPR': 4.2},
            #0.1: {'W': 1150, 'mdot': 4.8, 'i': 2000, 'HP_Nmech': 8200, 'Pt': 4.64, 'Tt': 450, 'turbPR': 4.8},
            #0.07: {'W': 1000, 'mdot': 3.8, 'i': 2000, 'HP_Nmech': 8200, 'Pt': 4.64, 'Tt': 450, 'turbPR': 5.0},
        },
        'MTO': {
            1.0: {'W': 8000, 'mdot': 40.0, 'i': 5000, 'HP_Nmech': 13689, 'Pt': 14.64, 'Tt': 527, 'turbPR': 3.0},
            0.9: {'W': 7600, 'mdot': 36.0, 'i': 4700, 'HP_Nmech': 13400, 'Pt': 14.64, 'Tt': 527, 'turbPR': 3.0},
            #0.8: {'W': 7200, 'mdot': 32.0, 'i': 4400, 'HP_Nmech': 13200, 'Pt': 14.64, 'Tt': 527, 'turbPR': 3.1},
            #0.7: {'W': 6800, 'mdot': 28.0, 'i': 4100, 'HP_Nmech': 13000, 'Pt': 14.64, 'Tt': 527, 'turbPR': 3.2},
            #0.6: {'W': 6200, 'mdot': 24.0, 'i': 3700, 'HP_Nmech': 12750, 'Pt': 14.64, 'Tt': 527, 'turbPR': 3.4},
            #0.5: {'W': 5600, 'mdot': 20.0, 'i': 3300, 'HP_Nmech': 12500, 'Pt': 14.64, 'Tt': 527, 'turbPR': 3.7},
            #0.4: {'W': 5000, 'mdot': 16.5, 'i': 2850, 'HP_Nmech': 12250, 'Pt': 14.64, 'Tt': 527, 'turbPR': 4.1},
            #0.3: {'W': 4300, 'mdot': 13.0, 'i': 2350, 'HP_Nmech': 12000, 'Pt': 14.64, 'Tt': 527, 'turbPR': 4.6},
            #0.2: {'W': 3600, 'mdot': 10.0, 'i': 1800, 'HP_Nmech': 11750, 'Pt': 14.64, 'Tt': 527, 'turbPR': 5.2},
            #0.1: {'W': 3476, 'mdot': 18.7, 'i': 4499, 'HP_Nmech': 12391, 'Pt': 14.64, 'Tt': 527, 'turbPR': 5.6},
            #0.07: {'W': 2900, 'mdot': 18.7, 'i': 4499, 'HP_Nmech': 12391, 'Pt': 14.64, 'Tt': 527, 'turbPR': 5.6},
        }
    }

    guess_map = guess_map_by_point[od_point]

    for k, frac in enumerate(load_fracs):
        target_hp = frac * (prob.model.settings.pwr_shaft_kW[od_point] * 1.341)
        prob.set_val(f'{od_point}.balance.pwr_target', target_hp, units='hp')

        if k == 0:
            g = guess_map[frac]
            prob.set_val(f'{od_point}.balance.W', g['W'], units='g/s')
            prob.set_val(f'{od_point}.balance.mdot_anode_in_excess', g['mdot'], units='g/s')
            prob.set_val(f'{od_point}.balance.i_sofc', g['i'], units='A/m**2')
            prob.set_val(f'{od_point}.balance.HP_Nmech', g['HP_Nmech'], units='rpm')
            prob.set_val(f'{od_point}.fc.balance.Pt', g['Pt'], units='psi')
            prob.set_val(f'{od_point}.fc.balance.Tt', g['Tt'], units='degR')
            prob[f'{od_point}.turb.PR'] = g['turbPR']
            prob[f'{od_point}.comp.PR'] = PR
        else:
            # continuation: only nudge sensitive variables
            g = guess_map[frac]
            #prob.set_val(f'{od_point}.balance.mdot_anode_in_excess', g['mdot'], units='g/s')
            #prob.set_val(f'{od_point}.balance.i_sofc', g['i'], units='A/m**2')
            prob.set_val(f'{od_point}.balance.W', g['W'], units='g/s')
        ...
        print(f"\nRunning {od_point} part-load case: {int(frac*100)}%")
        print(f"Target {od_point} LP shaft net power = {target_hp:.3f} hp")

        try:
            prob.run_model()
        except Exception as e:
            print(f"Simulation crashed at {od_point} {int(frac*100)}% load: {e}")
            continue

        actual_hp = prob.get_val(f'{od_point}.LP_shaft.pwr_net', units='hp')[0]
        target_hp2 = prob.get_val(f'{od_point}.balance.pwr_target', units='hp')[0]

        print(f"{od_point}.balance.pwr_target = {target_hp2:.6f} hp")
        print(f"{od_point}.LP_shaft.pwr_net   = {actual_hp:.6f} hp")
        print(f"Absolute error               = {abs(actual_hp - target_hp2):.6f} hp")
        print(f"Relative error               = {abs(actual_hp - target_hp2)/target_hp2*100:.6f} %")

        if frac == 1.0:
            save_data(prob, 'DESIGN', f"Results/DESIGN_TOC_basis_for_{od_point}.csv")
            print(f"Saved: Results/DESIGN_TOC_basis_for_{od_point}.csv")

        save_data(prob, od_point, f"Results/{od_point}_partload_{int(frac*100)}.csv")
        print(f"Saved: Results/{od_point}_partload_{int(frac*100)}.csv")

def run_cruise_and_mto_partload_sweeps():
    #run_partload_sweep_for_od_point('Cruise')
    run_partload_sweep_for_od_point('MTO')
...
def datafile_definition(prob, pt):
    ...
    data['Hex_h_hot_out_prelim'] = prob.get_val(f'{pt}.Hex. Fl_O_hot_pre: tot:h')[0]
    data['Hex_h_cold_out_prelim'] = prob.get_val(f'{pt}.Hex. Fl_O_cold_pre:tot:h')[0]
    #' Fl_O_hot_pre: tot:h'
    # ___SOFC___
...
if __name__=="__main__":
    ...
    run_cruise_and_mto_partload_sweeps()
    #run_TOC_partload_sweep_2()
    #plot_c ompressor_map_with_points()
